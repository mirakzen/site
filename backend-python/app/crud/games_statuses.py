...
from db.models import GamesStatuses
...
class GamesStatusesData:
    ...
    async def setup(self, session: AsyncSession) -> None:
        async with session.begin():
            db_data = await session.scalars(select(GamesStatuses))
            for row in db_data:
                self.data[row.id] = get_model_dict(row)
        self.resort()
        cfg.logger.info("Games statuses info was loaded to memory")
        ...
        async with self.lock:
            async with session.begin():
                await session.execute(
                    text(
                        f"TRUNCATE TABLE {GamesStatuses.__table__.name} RESTART IDENTITY;"
                    )
                )
                self.data = {}
                self.sorted_list = []
                return "Games statuses were erased"

    def resort(self) -> None:
        self.sorted_list = sorted(
            list(self.data.values()), key=lambda status: status["order"]
        )
        ...
        if raw:
            async with self.lock:
                ...
                for item in self.data.values():
                    item_record = {}
                    for tag in ("id", "code", "name", "order"):
                        item_record[tag] = item[tag]
                    result.append(item_record)
                return sorted(result, key=lambda status: status["order"])
        return self.sorted_list

    async def add(
        self, session: AsyncSession, elements: list[dto_games_statuses.NewElement]
    ) -> list[int]:
        if not elements:
            ...
            async with session.begin():
                ...
                for element in elements:
                    if element.order and 1 <= element.order <= (len(self.data) + 1):
                        await session.execute(
                            update(GamesStatuses)
                            .where(GamesStatuses.order >= element.order)
                            .values(order=GamesStatuses.order + 1)
                        )
                        for status_id in self.data:
                            if self.data[status_id]["order"] >= element.order:
                                self.data[status_id]["order"] += 1
                    else:
                    ...
                    element_id = (
                        await session.scalar(
                            insert(GamesStatuses)
                            .values(dicted_element)
                            .returning(GamesStatuses)
                        )
                    ).id
    ...
    async def update(
        self, session: AsyncSession, elements: list[dto_games_statuses.UpdatedElement]
    ) -> list[str]:
        if not elements:
            return HTTPabort(422, "Empty list")
        async with self.lock:
                    ...
                    if dicted_element.get("order") != None:
                        if 1 <= element.order <= len(self.data):
                            ...
                            if changed_orders:
                                await session.execute(
                                    update(GamesStatuses)
                                    .where(GamesStatuses.order.in_(changed_orders))
                                    .values(order=GamesStatuses.order + change)
                                )
                                for status_id in self.data:
                                    if self.data[status_id]["order"] in changed_orders:
                                        self.data[status_id]["order"] += change
                        else:
                            del dicted_element["order"]

                    await session.execute(
                        update(GamesStatuses)
                        .where(GamesStatuses.id == element.id)
                        .values(dicted_element)
                    )
    ...
    async def delete(
        self, session: AsyncSession, elements: list[dto_games_statuses.DeletedElement]
    ) -> list[bool]:
        if not elements:
            return HTTPabort(422, "Empty list")
        async with self.lock:
            async with session.begin():
                ...
                for element in elements:
                    ...
                    await session.execute(
                        delete(GamesStatuses).where(GamesStatuses.id == element.id)
                    )
                    await session.execute(
                        update(GamesStatuses)
                        .where(GamesStatuses.order > self.data[element.id]["order"])
                        .values(order=GamesStatuses.order - 1)
                    )
                    for status_id in self.data:
                        if (
                            self.data[status_id]["order"]
                            > self.data[element.id]["order"]
                        ):
                            self.data[status_id]["order"] -= 1
